...
and self-improvement protocols. It is the runtime copy; the version-controlled
source lives in the operational-judgment-skills repository as top-level
AGENTS.md.
...
- When IPv4 loopback succeeds but IPv6 loopback returns WSAEACCES 10013 or
  `ping ::1` reports a general failure, do not attribute the cause from loaded
  driver presence alone. Query the filters that match the exact traffic, then
  use a short `netsh wfp capture` to map the real `classifyDrop.filterId` back
  to its filter definition.
- Treat WFP filter IDs as session-local. Root-cause attribution requires a
  dynamic drop and its filter definition from the same capture or WFP state,
  including application path, user context, layer, sublayer, action,
  conditions, address, and port.
- Start WFP captures only in an elevated context, wrap them in `try/finally`,
  stop them in `finally`, and verify `netsh wfp capture status` reports no
  active capture.
- For VPN/TUN failures, distinguish ordinary domain/IP routing rules from TUN
  inbound and auto-route policy. A routing rule cannot override a terminating
  WFP block applied earlier at `ALE_AUTH_CONNECT_V4` or
  `ALE_AUTH_CONNECT_V6`.
- In this environment, v2rayN 7.16.8 with sing-box TUN, `AutoRoute=true`,
  `StrictRoute=true`, and `EnableIPv6Address=false` can generate an
  unconditional `sing-tun / block ipv6` filter. Use the supported v2rayN
  "启用 IPv6" setting instead of changing `Custom-Metratio-Whitelist` or
  stacking prefix-policy workarounds.
...
## 12. Local Computer Control

Avoid using screenshots or direct local-computer control to execute tasks whenever possible. When a task requires controlling the local computer, first design and provide the complete set of steps for the user to perform. Resume the task only after the user reports the results of those steps.
